...
OBS_TIME = 100  # Simulation time in minutes
...
class Machines(object):
    ...
    def pre_check_serve(self):
        yield self.env.timeout((self.service_time / 2) + random.expovariate(1))


def process_a(env, name, id_machine, allow_no_baggage):
    ...
    with id_machine.machine.request() as request:
        ...
        if allow_no_baggage and no_baggage > 5:
            ...
            env.process(process_b2(env, name, passenger_scan_machine, arrival_time, waiting_time, service_time, True))
        else:
    ...
    with belongings_box.machine.request() as request:
        ...
        print(
            'Zone B1: %s Finished preparing belongings %.2f.\t || Process B1 SERVICE TIME: %d / WAITING TIME: %d' % (
                name, env.now, ts, tw))
        env.process(process_b2(env, name, passenger_scan_machine, arrival_time, tw + waiting_time, ts + service_time, False))
        env.process(process_b3(env, name, baggage_scan_machine, arrival_time, tw + waiting_time, ts + service_time))


def process_b2(env, name, passenger_scanner, arrival_time, waiting_time, service_time, nobag):
    ...
    with passenger_scanner.machine.request() as request:
        ...
        if fail_flag > 5:
            ...
            env.process(process_d(env, name, additional_scanner, arrival_time, tw + waiting_time, ts + service_time))
        else:
            print(
...
def process_d(env, name, passenger_scanner, arrival_time, waiting_time, service_time):
...
def setup(env, allow_no_baggage):
    ...
    while True:
        ...
        env.process(process_a(env, 'Passenger %d' % number_of_passengers, id_check_booth, allow_no_baggage))
